...
public class GroovyDynamizeTransformer2 extends CompilationCustomizer {
    ...
    public void call(SourceUnit sourceUnit, GeneratorContext generatorContext, ClassNode classNode) throws CompilationFailedException {
        ...
        if (TRACE_COMPILE_METHODS) REPLLog.trace("{}: {} < {} ({})", classNode.isInterface() ? "INTERFACE" : "CLASS", classNode.getName(), String.join(", ", getAllSuperclasses(classNode).stream().map(ClassNode::getName).toList()), getNumberOfSuperclasses(classNode));
        ...
        for (MethodNode mn : selectDynamizableMethods(classNode)) {
            ...
            if (TRACE_COMPILE_METHODS) REPLLog.trace("    {}{}{}: {}<{}::{} ({})", mn.isStatic() ? "STATIC " : "", mn.getDeclaringClass().isInterface() ? "INTERFACE ": "", constructorNode != null ? "CONSTRUCTOR" : isTopDynamized(classNode) ? "SUPERSTUB" : "METHOD", classNode.getName(), mn.getDeclaringClass().getName(), mn.getTypeDescriptor(), mn.getModifiers());
